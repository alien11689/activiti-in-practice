...
class OrderRoute extends RouteBuilder{
    ...
    void configure() throws Exception {
        from("activiti:process-order:processOrder?copyVariablesToProperties=true").to("seda:asyncQueue");
        from("seda:asyncQueue").process(new Processor() {
            @Override
            void process(Exchange exchange) throws Exception {

                Thread.sleep(1000)
                if(exchange.getProperty("amount", Integer) < 100){
                    exchange.in.setHeader("orderFulfilled",true)
                }else{
                    exchange.in.setHeader("orderFulfilled", false)
                }
            }
        }).to("activiti:process-order:waitForProcessingEnd?copyVariablesFromHeader=true");
    }
}
